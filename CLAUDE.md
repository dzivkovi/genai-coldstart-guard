...
- [app/mock_backend.py](app/mock_backend.py) - simulates outcomes by switching on `request.route` (e.g. `mock:cold_start_timeout`, `mock:databricks_stopped`). Mock mode is the safe stand-in for the real endpoint's failure taxonomy; keep the route list in sync with README and `curl_examples.sh`.
- [app/databricks_client.py](app/databricks_client.py) - real backend. Flow: check endpoint state API -> `classify_state()` -> short-circuit if stopped/updating -> call `/invocations` **once** (no retry) -> map timeouts/transient 5xx to `warming`, auth/4xx to controlled messages. `build_databricks_payload()` and `invoke_databricks()` are the two functions to edit if the work endpoint expects a different schema/route.
...
2. **Conservative state classification.** `classify_state()` maps unknown Databricks states to `updating` (not `ready`) so unrecognised states never send real user workload to a possibly-not-ready endpoint. Real Databricks state payloads still need validation against a live endpoint - this is known follow-up debt.
