...
- [app/main.py](app/main.py) - the only two API routes (`POST /agentservice/agent/chat`, `POST /agentservice/agent/feedback`) plus `/health`. Dispatch is driven by the request `route` field: empty route -> real databricks backend; a `mock:*` route -> mock backend when `MOCK_ENABLED=true` (else a controlled 400 "Mock routes are disabled here."); any other route -> a controlled 400 "Unknown route: ...". `MOCK_ENABLED` defaults to false (secure by default, because `route` is a public request field; `.env.example` sets it true for local demos). `to_http_response()` is the key compatibility shim (see below).
