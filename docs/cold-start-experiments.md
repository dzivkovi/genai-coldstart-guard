...
Observed against a live custom model serving endpoint (a self-hosted LLM used as a stand-in). Note: "LLM endpoint" is not a distinct Databricks type - it's a custom model, the same type as a no-inference routing service, so the behaviour transfers:

1. **The cold-start response surface is not deterministic - classify on the outcome class.** On this self-deployed custom LLM endpoint, the first 2-3 invocations after idle returned a fast `429` ("starting"), then succeeded. But Databricks does not document a single cold-start response: a cold (zero-capacity) endpoint can also queue the request and return a slow `200`, or time out if warm-up exceeds the client/gateway limit (cold start can be minutes; hard cap 597s). The fast `429` is observed behavior (queue/admission with no replica ready), **not a documented contract**, and it depends on load, concurrency config, and model load time. So treat `429`, timeout, and transient `5xx` all as `warming` (with `Retry-After`), and **measure the actual surface per endpoint type** rather than hard-coding one signal. Practical upside: a fast `429` lets you show "starting" in ~1s; a slow queue-wait needs the timeout fallback.
